# How to develop a Biskuit theme
...
**Note** The [completed theme](https://github.com/biskuit/example-theme) can be found on Github.

## Getting started

For this tutorial, we assume you have a running Biskuit installation on a local server environment. If that is not the case, [download](https://biskuit.org/download) a Biskuit installation package and quickly [install](../getting-started/installation.md) it. Then login to the admin area and have a look at the Theme section of the integrated Marketplace.

The Biskuit Marketplace features our [Hello Theme](https://biskuit.org/marketplace/package/biskuit/theme-hello), a blueprint for developing themes, including code examples and a general foundation to help you get started.
...
First of all install the theme from the Biskuit marketplace and have a look at the overall file structure. When installed, the *Hello* theme is located in `packages/biskuit/theme-hello`. If you develop your own themes in the future, we recommend to install a simple theme on the side for reference, like the default [Theme One](https://biskuit.org/marketplace/package/biskuit/theme-one). That way you can compare structural elements and get some inspiration. For this tutorial though, we only need Hello theme.
...
composer.json             // package definition so that Biskuit recognizes the theme
...
1. Copy all files from `packages/biskuit/theme-hello` to `packages/your-name/your-theme` (you will need to create these folders).
2. Open `composer.json` and replace `"name": "biskuit/theme-hello",` with `"name": "your-name/your-theme"`. ALso change `"title": "Hello"` to `"title": "Your theme"`.
...
### CSS
Hello theme doesn't contain any styles. The included `css/theme.css` file lists all CSS classes that are rendered by the Biskuit core extensions without additional styling. You can add your own CSS to these classes.

The Biskuit admin interface and the Biskuit default themes are built with the [UIkit front-end framework](http://getuikit.com/). So you might want to consider using it for your project as well. In the following example we will be building our theme using UIkit. If you use another framework or no framework at all, the basic approach of including CSS is similar.
...
To make sure the file is loaded by Biskuit, open the main layout file of the theme which is located at `theme-hello/views/template.php`.
...
There are a number of possible file structure setups. In the following passage we suggest the structure that is also used for the official Biskuit themes. While it might look like many steps when you create a theme for the first time, in our experience this setup allows for well structured code, easy UIkit customizations and a comfortable way to keep UIkit up to date using Bower.
...
Open your theme in a new console tab (for example `cd biskuit/packages/theme-hello`) and run `npm install`, `bower install` and `gulp`.
...
The currently loaded version of jQuery and UIkit depend on the current version of Biskuit. With new releases of Biskuit, the versions of these libraries will continually be updated. While this allows for always having a current version available, a potential downside would be that you need to make sure your code also works for the new versions of these libraries.
...
The page *content* is nothing other than Biskuit's system output. That means that the content of any page you create will be rendered in this area.
...
To navigate through any site, you first need to set up a *menu*. For this purpose, Biskuit provides different menu positions that allow users to publish menus in several locations of the theme markup.
...
<figure class="uk-thumbnail">
    ...
    <figcaption class="uk-thumbnail-caption">A menu can be published to the defined positions in the Biskuit Site Tree.</figcaption>
</figure>


#### Step 2

With the concept of modularity in mind, Biskuit renders position layouts in separate files. For the navigation, create the `views/menu-navbar.php` file containing the following:
...
Biskuit uses [Vue.js](https://vuejs.org/) to build its administration interface. Here is a [Video tutorial](https://youtu.be/3gPGyhTroSA?list=PL2rU5GxE-MQ7aYIcxTmDh4-BTHRM-9lto) on Vue.js in Biskuit.
...
In the freshly created file `site-theme.vue` we add the option which will be displayed in the Biskuit administration.
...
Whenever you now apply changes to the Vue component, you need to run this task again. Alternatively, you can run `webpack --watch` or `webpack -w` which will stay active and automatically recompile whenever you change the Vue component. You can quit this command with the shortcut *Ctrl + C* For more information on Vue and Webpack, take a closer look at [this doc](https://biskuit.org/docs/developer/vuejs-and-webpack).
...
Widget positions allow users to publish widgets in several locations of your theme markup. They appear in the *Widgets* area of the Biskuit admin panel and can be selected by the user when setting up a widget.
...
Now that we have made the new position known to Biskuit, we need also to create a position renderer. We could skip this step and use a default renderer, but then all widgets would simply render below each pther. So to lay out the widgets in a grid, create the file `views/position-grid.php`:
...
First, we need to create the file `node-theme.vue` inside the folder `app/components`. Here we add the option which will be displayed in the Biskuit administration. Settings that are stored in this file can be applied to each page separately by entering the appropriate item in the site tree and clicking the *Theme* tab.
...
A theme can also customize the way how Biskuit renders static pages and blog posts. You simply overwrite system view files to apply your own layout. To do so, create corresponding folders inside your theme to mimic the original structure of the Biskuit system and put the template files there:
...
In this tutorial, you have learned the basic knowledge and tools to create themes for Biskuit. Let us summarize which topics we have covered.

**Note** The [completed theme](https://github.com/biskuit/example-theme) can be found on Github.

- You are now familiar with the **file structure** of a Biskuit theme. The main entry point for configuration and custom code is the theme's `index.php`, the main template file is located at `views/template.php` inside the theme.
...
- **Widgets and menus** are managed from the Biskuit admin area and are rendered in special positions of your theme. You have learned how to create these positions and how to change the default widget rendering.
...
With these skills you are now in a position to create Biskuit themes, both for client projects and also for the Biskuit marketplace.
